 ...
 * this link
 * https://github.com/ggchappell/cs411-2025-03/blob/main/merge_sort.cpp These
 * functions were modified in the course of this assignment, modifications are
...
size_t stableMerge(FDIter first, FDIter middle, FDIter last) {
  ...
  // MODIFICATION: add a static cast to size_t to silence compiler warning
  // -Wsign-conversion
  ...
  while (in1 != middle && in2 != last) {
    if (*in2 < *in1) // Must do comparison this way, to be stable.
    {
      ...
      inversions += static_cast<size_t>(distance(first, in2) -
                                        distance(begin(buffer), out));
      *out++ = std::move(*in2++);
    } else {
      *out++ = std::move(*in1++);
    }
...
template <typename FDIter> size_t mergeSort(FDIter first, FDIter last) {
